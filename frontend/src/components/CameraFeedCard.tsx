...
function LiveDetectionBox({
  ...
  isPersonCam: boolean;
  isCrossCam?: boolean;
  isCriminalCam?: boolean;
}) {
  const [x, y, w, h] = obj.bbox;
  const isPlate = obj.cls === "plate" || Boolean(obj.plate);
  const isFace = obj.cls === "face";
  const isAuthorized = isFace && (obj.watchlist || obj.status === "AUTHORIZED");
  const isIntruder = isFace && (!obj.watchlist || obj.status === "INTRUDER");
  const isTrespasser = Boolean(obj.is_trespasser) || obj.state === "intrusion" || (typeof obj.label === "string" && obj.label.includes("TRESPASSER"));
  const isCriminal = Boolean(obj.is_criminal) || isCriminalCam || obj.status === "CRIMINAL_WARNING" || (typeof obj.label === "string" && obj.label.includes("CRIMINAL"));
  const isCrossCameraMatch =
    Boolean(isCrossCam) ||
    obj.cross_cam === "CAM-01" ||
    Boolean(obj.cross_camera_alert) ||
    (typeof obj.label === "string" && obj.label.includes("CAM-01"));

  let color = "#3B82F6";
  if (isCriminal && (obj.cls === "person" || isCriminalCam)) color = "#DC2626";
  else if (isTrespasser) color = "#EF4444";
  else if (isCrossCameraMatch && (obj.cls === "person" || isCrossCam)) color = "#A855F7";
  else if (isPersonCam) color = "#22C55E";
  else if (isPlate) color = "#FACC15";
  else if (isAuthorized) color = "#22C55E";
  else if (isIntruder) color = "#EF4444";

  let label = obj.label;
  if (isCriminal && (obj.cls === "person" || isCriminalCam)) {
    label = `🚨 WANTED CRIMINAL #${obj.id} [CAM-04 Trespasser]`;
  } else if (isTrespasser) {
    label = `🚨 TRESPASSER #${obj.id} [Room Intrusion]`;
  } else if (isCrossCameraMatch && (obj.cls === "person" || isCrossCam)) {
    label = `🔁 Person #${obj.id} [Appeared in CAM-01]`;
  } else if (isPersonCam) {
    label = `Person #${obj.id}`;
  } else if (isPlate) {
    label = `🪪 ${obj.label || obj.plate}`;
  } else if (isFace) {
    if (isAuthorized) {
      label = `✓ ${obj.label || "Authorized"} [${obj.designation || "Personnel"}]`;
    } else {
      label = `⚠️ INTRUDER (Unauthorized)`;
    }
  }
  ...
  return (
    <div
      style={{
        ...
        pointerEvents: "none",
        zIndex: 3,
        boxShadow: isCriminal && (obj.cls === "person" || isCriminalCam)
          ? "0 0 14px rgba(220,38,38,0.9), inset 0 0 10px rgba(220,38,38,0.4)"
          : isTrespasser
          ? "0 0 12px rgba(239,68,68,0.85), inset 0 0 8px rgba(239,68,68,0.35)"
          : isCrossCameraMatch
          ? "0 0 10px rgba(168,85,247,0.7), inset 0 0 8px rgba(168,85,247,0.25)"
          : isIntruder
          ? "0 0 10px rgba(239,68,68,0.7), inset 0 0 8px rgba(239,68,68,0.3)"
          : isAuthorized
    ...
    >
      <div
        style={{
          ...
          left: 0,
          padding: "1px 5px",
          background: isCriminal && (obj.cls === "person" || isCriminalCam)
            ? "rgba(40,0,0,0.95)"
            : isCrossCameraMatch
            ? "rgba(30,10,50,0.9)"
            : "rgba(0,0,0,0.85)",
          ...
          letterSpacing: isPlate ? "0.06em" : "0.02em",
          whiteSpace: "nowrap",
        }}
      >
        {label}
...
function DetectionAlertPanel({
  ...
  const list = isPersonCam
    ...
    : isCrossCam || isCriminalCam
    ? objs.filter((o) => o.cls === "person" || VEHICLE_CLASSES.has(o.cls) || o.cls === "plate")
    : objs.filter((o) => VEHICLE_CLASSES.has(o.cls) || o.cls === "plate");
  const plates = objs.filter((o) => o.cls === "plate" || Boolean(o.plate));

  const alerts = objs
    .filter((o) => o.confidence >= 0.6 || o.is_criminal || o.is_trespasser)
    .slice(0, 6)
    .map((o) => {
      if (isCriminalCam && (o.is_criminal || o.cls === "person")) return `🚨 CRIMINAL: #${o.id} on Wanted List (CAM-04 Room Trespasser)`;
      if (isCrossCam && o.cls === "person") return `🔁 Person #${o.id} appeared in CAM-01`;
      if (o.cls === "plate" || o.plate) return `Plate read ${o.label || o.plate}`;
  ...
  const accent = isCrossCam ? "#A855F7" : isPersonCam ? "#22C55E" : "#3B82F6";
...
/** CAM06 live facial-recognition panel: watchlist matches, intruder alerts, enrollment. */
function FaceAlertPanel({
  ...
  const wl = faces.filter((o) => o.watchlist || o.status === "AUTHORIZED");
  const intruders = faces.filter((o) => !o.watchlist || o.status === "INTRUDER");
  ...
  const countTile = (label: string, value: number, color: string, pulse = false) => (
    <div style={{ flex: 1, minWidth: 0, textAlign: "center", padding: "4px 2px", background: "rgba(255,255,255,0.02)", borderRadius: 4, border: "1px solid var(--border)" }}>
      <div
        style={{
          display: "flex",
          alignItems: "center",
      ...
      <div style={{ fontSize: 6, fontWeight: 700, color: "var(--text-3)", letterSpacing: "0.05em", marginTop: 3 }}>
        {label}
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        />
        <span style={{ fontSize: 8.5, fontWeight: 700, color: "var(--text-2)", letterSpacing: "0.08em" }}>
      ...
      <div style={{ padding: "8px 12px", display: "flex", gap: 6, borderBottom: "1px solid var(--border)" }}>
        ...
        {countTile("AUTHORIZED", wl.length, "#22C55E", true)}
        {countTile("INTRUDERS", intruders.length, "#EF4444", true)}
      </div>
      ...
      <div style={{ padding: "8px 12px", borderBottom: "1px solid var(--border)" }}>
        ...
        >
          <UserPlus size={10} /> ENROLL PERSONNEL / ROSTER
        </button>
      </div>

      {/* Live Match & Intruder Alert Feed */}
      <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "10px 12px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
        <span style={{ fontSize: 7, fontWeight: 700, color: "var(--text-3)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
          Access Feed & Real-time Alerts
        </span>
        {faces.length === 0 && alerts.length === 0 ? (
          <div style={{ fontSize: 7.5, color: "var(--text-3)", padding: "6px 8px", background: "rgba(255,255,255,0.02)", borderRadius: 4, border: "1px solid var(--border)" }}>
            Scanning CAM-06 sector… Unrecognized faces will be flagged as Unauthorized Intruders.
          </div>
        ) : (
          <>
            {/* Live active intruders if any */}
            {intruders.map((o) => (
              <div
                key={`intruder-${o.id}`}
                style={{
              ...
              >
                ...
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: 7.5, fontWeight: 700, color: "#F87171" }}>
                    ⚠️ INTRUDER (Unauthorized)
                  </div>
                  <div style={{ fontSize: 6.5, color: "rgba(255,255,255,0.6)" }}>
                    Face ID #{o.id} · Not in Authorized Roster
                  </div>
                </div>
              </div>
            ))}

            {/* Live authorized faces */}
            {wl.map((o) => (
              <div
                key={`auth-${o.id}`}
                style={{
              ...
              >
                <ShieldCheck size={10} style={{ color: "#22C55E", flexShrink: 0 }} />
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: 7.5, fontWeight: 700, color: "#86EFAC" }}>
                    ✓ {o.label}
                  </div>
                  <div style={{ fontSize: 6.5, color: "#38BDF8" }}>
                    {o.designation || "Authorized Personnel"}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div style={{ padding: "6px 12px", borderTop: "1px solid var(--border)", fontSize: 7, color: "var(--text-3)", marginTop: "auto" }}>
        {feed.id.toUpperCase().replace("-", " ")} · {feed.sector} · ACCESS CONTROL
      </div>

      {/* Personnel Enrollment Modal */}
      <PersonnelEnrollmentModal
        isOpen={modalOpen}
...
export default function CameraFeedCard({
  ...
  const streamId = feed.detectionStream || feed.id || "";
  ...
  const isCrossCam = streamId === "cam-03";
  const isFenceCam = streamId === "cam-04";
  const isCriminalCam = streamId === "cam-05";
  ...
  const visibleObjects = liveFrame
    ? isFaceCam
      ? liveObjects.filter((o) => o.cls === "face")
      : isPersonCam
      ? liveObjects.filter((o) => o.cls === "person")
      : isCrossCam
      ? liveObjects.filter((o) => o.cls === "person" || VEHICLE_CLASSES.has(o.cls) || o.cls === "plate")
      : isCriminalCam
      ? liveObjects.filter((o) => o.cls === "person" || VEHICLE_CLASSES.has(o.cls) || o.cls === "plate")
      : isFenceCam
      ? liveObjects.filter((o) => o.cls === "person")
      : liveObjects.filter((o) => VEHICLE_CLASSES.has(o.cls) || o.cls === "plate" || o.cls === "person")
    : [];
  ...
  const renderSurface = () => (
    <div style={{ position: "absolute", inset: 0 }}>
      ...
      {/* ── Top Warning Banner for CAM-05 Criminal Warning & CAM-04 Trespasser ── */}
      {isCriminalCam && (Boolean(liveFrame?.criminal_warning_alert) || visibleObjects.some((o) => o.is_criminal)) && (
        <div
          style={{
            position: "absolute",
            top: 26,
        ...
        >
          <span style={{ fontSize: 7.5, fontWeight: 800, color: "#FFF", letterSpacing: "0.04em", display: "flex", alignItems: "center", gap: 4 }}>
            🚨 WANTED CRIMINAL IDENTIFIED · CAM-04 ROOM TRESPASSER
          </span>
          <span style={{ fontSize: 7, fontWeight: 700, color: "#FECACA", background: "rgba(0,0,0,0.4)", padding: "1px 4px", borderRadius: 2 }}>
            CRITICAL
          </span>
        </div>
      )}

      {isFenceCam && (liveFrame?.counts?.intrusion ?? 0) > 0 && (
        <div
          style={{
            position: "absolute",
            top: 26,
        ...
        >
          <span style={{ fontSize: 7.5, fontWeight: 800, color: "#FFF", letterSpacing: "0.04em", display: "flex", alignItems: "center", gap: 4 }}>
            🚨 RESTRICTED ROOM BREACH · ACTIVE TRESPASSER DETECTED
          </span>
          <span style={{ fontSize: 7, fontWeight: 700, color: "#FECACA", background: "rgba(0,0,0,0.4)", padding: "1px 4px", borderRadius: 2 }}>
            ALERT
          </span>
            ...
            {isFenceCam ? (
              ...
              visibleObjects.map((obj, i) => (
                <LiveDetectionBox
                  ...
                  isCrossCam={isCrossCam}
                  isCriminalCam={isCriminalCam}
                />
              ))
            )}
          </div>
